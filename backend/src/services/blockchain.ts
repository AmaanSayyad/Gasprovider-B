...
export class BlockchainService {
  ...
  constructor() {
    // Initialize signer from private key (env only — never hardcode)
    const privateKey = getDistributorPrivateKey();
    if (!privateKey) {
      console.warn(
        "⚠️  DISTRIBUTOR_PRIVATE_KEY / PRIVATE_KEY / RELAYER_PRIVATE_KEY not set. Contract calls will fail."
      );
    } else {
      // We'll create signers per-chain when needed
      console.log("✅ Distributor private key loaded from env");
    }
  }
  ...
  private getSigner(chainId: number): ethers.Wallet {
    ...
    if (!privateKey) {
      throw new Error(
        "DISTRIBUTOR_PRIVATE_KEY (or PRIVATE_KEY / RELAYER_PRIVATE_KEY) not set in environment"
      );
    }
