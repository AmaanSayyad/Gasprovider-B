...
async function main() {
  ...
  const defaults: Record<number, { USDC: string; WETH: string; ROUTER: string }> = {
    ...
    // Note: Using mock addresses for testnet - deploy mock tokens first if needed
    114: {
      USDC: "0x0000000000000000000000000000000000000000", // Will deploy mock USDC
      WETH: "0x0000000000000000000000000000000000000000", // Will use WFLR (Wrapped FLR)
      ROUTER: "0x0000000000000000000000000000000000000000", // Will deploy mock router
    },
  };
